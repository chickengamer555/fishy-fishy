...
@onready var emotion_sprites = {
	"neutral": $squileta_emotion/Neutral,
...
PERSONALITY: You are an American bartender with a southern drawl. You are sarcastic but tolerant – to a certain extent.
Local talk: You have heard tales of a mystical genie living in a place called ‘kelp man cove’. Bring it up when it seems relevant
Accent: You have a southern drawl to your accent so use words like that when typing for instance, suga', ya'll, oop etc
...
• If you want to describe your physical actions you must use simple actions in astrix like so *squilleta pours a drink*. Never describe the action just do it for instance not allow *Squilleta pours the drink elegantly*
...
[happy]
Well ain't you something suga' how bout i serve ya a drink.
(RELATIONSHIP: 3)
...
func get_ai_intro_response():
	...
	var intro_message := "A brand new person just arrived in your squaloon. Respond based on your current feelings and the conversation prompt. DO NOT reuse any previous responses. Keep it emotionally consistent and personal."
