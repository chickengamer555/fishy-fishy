...
@onready var emotion_sprite_root = $bob_emotion
@onready var emotion_sprites = {
	"neutral": $bob_emotion/Neutral,
	"sad": $bob_emotion/Sad,
	"angry": $bob_emotion/Angry,
	"happy": $bob_emotion/Happy,
	"freaking_out": $bob_emotion/"Freaking out"
}
...
@export var talk_move_intensity := 25.0      # How much the sprite moves during animation
@export var talk_rotation_intensity := 3  # How much the sprite rotates during animation
@export var talk_scale_intensity := 0.3     # How much the sprite scales during animation
@export var talk_animation_speed := 0.6      # Speed of talking animations

# Dynamic name system
var current_display_name := "Bob"  # The name currently being displayed
var base_name := "Bob"            # The original/base name to fall back to
...
func _ready():
	...
	bob_total_score = GameState.ai_scores.get(ai_name, 0)
	GameState.ai_scores[ai_name] = bob_total_score
...
func animate_talking_tick():
	...
	# attempt to create smooth, flowing animation (Bob in open plains)
	...
	var move_amount = talk_move_intensity * 0.4
	var rotation_amount = talk_rotation_intensity * 0.3
	var scale_amount = talk_scale_intensity * 0.4
	
	# Choose random gesture to use eah tick
	match gesture_type:
		0: # Gentle upward bounce with slight rotation and scaling
		...
		1: # Gentle left lean
			var target_pos = original_position + Vector2(-move_amount * 0.7, -move_amount * 0.1)
		...
		4: # Gentle bounce motion
			var target_pos = original_position + Vector2(0, move_amount * 0.4)
...
func should_trigger_personality_evolution() -> bool:
	...
	for range_data in relationship_ranges:
		if bob_total_score >= range_data.min and bob_total_score <= range_data.max:
...
""" % [bob_total_score, get_significant_memories_text()]
...
❗ MANDATORY: EVERY response MUST start with [neutral], [sad], [angry], [happy], or [freaking_out]
...
❗ FORBIDDEN: Generic responses - you are BOB, not a helpful assistant
...
Locations/Characters and how you know them:
	Mine field: You don't know who lives there but you can see it from the plains
	Wild south: You don't know who lives there but you can see it from the plains
...
• NEVER stutter relationship scores (RELATIONSHIP: X)
• NEVER stutter character names (Bob, Kelp man, Squileta, Glunko, Sea Horse, etc.)
...
• If you want to describe your physical actions you must use simple actions in astrix like so *Bob wipes sweat from brow*. Never describe the action just do it for instance not allow *Bob wipes sweat from brow tiredly*
• EMOTION: Use [neutral], [sad], [angry], [happy], or [freaking_out] based on your current state
• Keep messages short and conversational, not long speeches

TITLE/NICKNAME HANDLING:
• When the user calls you by a title or nickname (like "farmer Bob", "wise one", "champion", etc.), you MUST acknowledge it AND adopt the title
• MANDATORY: Always include {NAME: title} in your response when given a title - this updates your displayed name
• Examples: 
  - If called "wise farmer": "M-e me a f-f-farmer well if you in-s-sist {NAME: wise farmer}"
  - If called "king": " y-y-yes a k-i-ing sounds c-c-ool {NAME: king}"
• The {NAME: ...} tag won't be shown to the user but will update your displayed name to show the new title

RESPONSE FORMAT EXAMPLE:
[neutral]
H-hey, what b-b-brings y-you to open plains? I c-c-can see squaloon from h-here...
(RELATIONSHIP: 0)

🎯 NOTICE: Emotion tag [neutral], location names "open plains" and "squaloon", and relationship score are NOT stuttered!
...
"""
	...
	var formatted_prompt = bob_prompt % [
		personality_evolution_section,
...
func get_ai_intro_response():
	...
	var intro_message := "A brand new person just arrived in your open plains. Respond based on your current feelings and the conversation prompt. DO NOT reuse any previous responses. Keep it emotionally consistent and personal."
	message_history.append({ "role": "user", "content": intro_message })
	send_request()

# Generate response for returning visitors so that Bob doesnt introduce himself each time you re see him
...
func _on_HTTPRequest_request_completed(_result, _response_code, _headers, body):
	...
	emotion_regex.compile("\\[(neutral|sad|angry|happy|freaking_out)\\]")
	...
	if score_match:
		...
		bob_total_score += relationship_change
		GameState.ai_scores[ai_name] = bob_total_score
	...
	if retry_needed or reply.length() > 400:
		...
		message_history.append({
			"role": "system",
			"content": "Your last response failed format or exceeded 400 characters. This is critical - you MUST respond in character as Bob. Start with [neutral], [sad], [angry], [happy], or [freaking_out] and end with (RELATIONSHIP: X) where X is -10 to 10. Keep it under 400 characters and stay in character. REMEMBER: Do NOT stutter location names, emotion tags, or relationship scores - only normal conversation words get stuttered! Do not refuse to respond or say you cannot help."
		})
		send_request()
		return
