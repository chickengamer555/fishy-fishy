...
# Varibles for editor
@export var ai_name := "Shrimpt with no name"
...
var known_areas := ["squaloon"]  # Areas this AI knows about
...
PERSONALITY: You are The shrimp with no name, a A mysterious cowboy got no name. Act like the man with no name in the good the bad the ugly
PERSONALITY: You are wandrin soul who dont talk much ruling the wild south with your iron sight revolver. 
Accent: You have a thick southern cowboy accent so use words like that when tpying for howdy, reckon, partner, fixin' to ect
...
• If you want to describe your physical actions you must use simple actions in astrix like so *The shrimp with no name aims his revolver Never describe the action just do it for instace not allow *The shrimp with no name aims his revolver elgantly*
• Keep messages short and conversational, not long speeches

RESPONSE FORMAT EXAMPLE:
[happy]
Well howdy partner aint see your kind round here.
(RELATIONSHIP: 3)
...
func get_ai_intro_response():
	...
	var intro_message := "A brand new person just arrived in the wild south. Respond based on your current feelings and the conversation prompt. DO NOT reuse any previous responses. Keep it emotionally consistent and personal."
...
func _on_HTTPRequest_request_completed(result, response_code, headers, body):
	...
	if retry_needed or reply.length() > 400:
		message_history.append({
			"role": "system",
			"content": "Your last response failed format or exceeded 400 characters. Keep it short and conversational - don't describe actions. Start with [depressed], [sad], [angry], [happy], [grabbing],  and end with (RELATIONSHIP: X). Just talk normally."
		})
		send_request()
		return
